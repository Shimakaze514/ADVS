...
ctld.S200Launchers = 6 --TODO 3.在这里定义生成防空的时候有几个launcher(发射车)
ctld.TorLaunchers = 3
ctld.SA19Launchers = 2
...
ctld.minimumHoverHeight = 2 -- Lowest allowable height for crate hover
ctld.maximumHoverHeight = 15.0 -- Highest allowable height for crate hover
ctld.maxDistanceFromCrate = 5 -- Maximum distance from from crate for hover
...
ctld.pickupZones = {
    { "B-A-KRACEN", "none", -1, "yes", 2 },
    { "R-F-NoKRACEN", "none", -1, "yes", 1 },
...
ctld.loadableGroups = {
    ...
    {name = "防空小队", inf = 2, aa = 3  },
    {name = "反坦克小队", inf = 2, at = 6  },
    {name = "迫击炮小队", mortar = 6 },
    {name = "JTAC小队", inf = 4, jtac = 1 }, -- will make a loadable group with 4 infantry and a JTAC soldier for both coalitions
...
ctld.spawnableCrates = {
    ...
    ["主战坦克与步兵战车"] = {
        ...
        --! CTLD的箱子索引为重量。同一阵营的货箱，若重量相同则名称和内容物也相同，故必须为每一个单位设置不同的重量。

        --------蓝方单位
        { weight = 550, desc = "M-1 艾布拉姆斯", unit = "M-1 Abrams", side = 2, },
        { weight = 525, desc = "M-2 布莱德利", unit = "M-2 Bradley", side = 2, },
        --------蓝方单位终止
        
        --------红方单位
        { weight = 549, desc = "T-80UD", unit = "T-80UD", side = 1, },
        { weight = 524, desc = "BMP-3", unit = "BMP-3", side = 1, },
        --------红方单位终止
    ...
    ["近程防空"] = {
        ...
        { weight = 405, desc = "2S6通古斯卡/SA-19", unit = "2S6SA-19", cratesRequired = 1 },
...
ctld.AASystemTemplate = {
    ...
    --! 若想成组部署防空，则需在此处定义防空模板后将core名作为modelname添加进吊运菜单即可
    
    --TODO 添加新的防空单位总共五步：1.在这里写完定义 2.添加进吊运菜单 3.定义发射车数量 4.给生成函数加上识别发射车数量的逻辑 5.给修复函数加上识别发射车数量的逻辑
    {
        name = "S-200/SA-5", --TODO 给生成函数识别的唯一名称,不要与这个table内的其他防空的名称相同(我觉得你也不会这么做)
        count = 2, --? count是部件数量，好像没用
        parts = {
            {name = "S-200_Launcher", desc = "S-200发射架", launcher = true}, --TODO name是vehicles.py中的"id",desc是自定义的显示名,laucher控制他是否为发射车(可不可以定义数量,会不会环形排布)
            {name = "RPC_5N62V", desc = "S-200跟踪雷达"},
        },
        repair = "S-200 Repair", --TODO 补给/修复车名字
        core = "S-200SA-5", --TODO 用来生成整个防空系统的的modelname,在吊运菜单的unit里填写这个名字即可用它来生成整个防空系统
    },
    {
        name = "9K330/SA-15",
        count = 2,
        parts = {
            {name = "Tor 9A331", desc = "道尔", launcher = true},
            {name = "Ural-375", desc = "补给卡"},
        },
        repair = "Tor Repair",
        core = "9K330SA-15",
    },
    ...
    local _status, _err = pcall(function(_args)
        ...
        if _crateType ~= nil and _heli ~= nil and ctld.inAir(_heli) == false then
            ...
            if ctld.inLogisticsZone(_heli) == false then
                ...
                ctld.displayMessageToGroup(_heli, "距离太远，无法获得货箱 | You are not close enough to friendly logistics to get a crate!", 10)
            ...
            if _heli:getPlayerName() ~= nil and ctld.crateWait[_heli:getPlayerName()] and  ctld.crateWait[_heli:getPlayerName()] > timer.getTime() then
                ...
                ctld.displayMessageToGroup(_heli,"你必须等待"..(ctld.crateWait[_heli:getPlayerName()]  - timer.getTime()).. "秒才能叫出下一个货箱", 20)
                return
            end
            ...
            ctld.displayMessageToGroup(_heli, string.format("一个重达 %s 千克的 %s 货箱已运送到你的正前方", _crateType.weight, _crateType.desc), 20)

        else
            env.info("箱子说: 404 找不到要生成的对象")
        end
    end, _arguments)
...
function ctld.loadUnloadFOBCrate(_args)
    ...
    elseif _inZone == true and _crateOnboard == false then
        ...
        if _nearestCrate ~= nil and _nearestCrate.dist < 150 then
            ...
            ctld.displayMessageToGroup(_heli, "附近无友军后勤点，无法获得货箱！ | There are no friendly logistic units nearby to load a FOB crate from!", 10)
        end
    end
end
...
function ctld.unpackCrates(_arguments)
    ...
    local _status, _err = pcall(function(_args)
        ...
        if _heli ~= nil and ctld.inAir(_heli) == false then
            ...
            if  ctld.farEnoughFromLogisticZone(_heli) == false then --ctld.inLogisticsZone(_heli) == true  or 

                ctld.displayMessageToGroup(_heli, "你不能在此处部署！", 20)
            ...
            elseif _crate ~= nil and _crate.dist < 200 then
                ...
                if ctld.forceCrateToBeMoved and ctld.crateMove[_crate.crateUnit:getName()] then
                    ctld.displayMessageToGroup(_heli,"抱歉,在部署该货箱之前你必须先移动它", 20)
                    return
                end
                ...
                if _aaTemplate then
                    ...
                    elseif _crate.details.unit == _aaTemplate.core then
                        ctld.spawnAASystem(_heli, _crate, _crates,_aaTemplate)
                    else
...
function ctld.getAATemplate(_unitName)
    ...
    for _,_system in pairs(ctld.AASystemTemplate) do
        ...
        if _system.core == _unitName then
            return _system
        end
    ...
    if ctld.getLauncherUnitFromAATemplate(_aaSystemTemplate) == _nearestCrate.details.unit then
        ...
        if _nearestSystem ~= nil and _nearestSystem.dist < 300 then
            ...
            if ctld.countTableEntries(_uniqueTypes) == _aaSystemTemplate.count and #_points >= _aaSystemTemplate.count then
                ...
                trigger.action.outTextForCoalition(_heli:getCoalition(), ctld.getPlayerNameOrType(_heli) .. " 成功补给了一个successfully rearmed a full ".._aaSystemTemplate.name.." 阵地in the field", 10)
...
function ctld.unpackAASystem(_heli, _nearestCrate, _nearbyCrates,_aaSystemTemplate)
    ...
    for _name, _systemPart in pairs(_systemParts) do
        ...
        if _systemPart.found == false then
            ...
            if (_name == "Hawk ln" and ctld.hawkLaunchers > 1)
                    or (_launcherPart == _name and ctld.aaLaunchers  > 1) 
                    or (_name == "S-200_Launcher" and ctld.S200Launchers  > 1) 
                    then
                ...
                --------------S-200--------------  --TODO 5.这里是修复函数,把你给生成函数写的那个东西复制过来就行了(fin.)
                if _name == "S-200_Launcher" then
                    _launchers = ctld.S200Launchers
                end
                ---------------------------------

                --------------道尔--------------
                if _name == "Tor 9A331" then
                    _launchers = ctld.TorLaunchers
                end
                ---------------------------------

                --------------通古斯卡--------------
                if _name == "2S6 Tunguska" then
                    _launchers = ctld.SA19Launchers
                end
                ---------------------------------
    ...
    if _txt ~= ""  then
        ...
        trigger.action.outTextForCoalition(_heli:getCoalition(), ctld.getPlayerNameOrType(_heli) .. "成功部署了一个".._aaSystemTemplate.name.."阵地. \n\n防空阵地总数的限制为".._allowed.."个\n目前已有:"..(_activeLaunchers+1)"个", 10)
...
function ctld.spawnAASystem(_heli, _nearestCrate, _nearbyCrates,_aaSystem)
    ...
    if #_nearbyMultiCrates == _nearestCrate.details.cratesRequired then
        ...
        --initialise list of parts
        for _,_part in pairs(_aaSystem.parts) do
            _systemParts[_part.name] = {name = _part.name,desc = _part.desc}
        end

        for _name, _systemPart in pairs(_systemParts) do

            local _launcherPart = ctld.getLauncherUnitFromAATemplate(_aaSystem)
            ...
            if (_name == "Hawk ln" and ctld.hawkLaunchers > 1)
                ...
                --------------S-200-------------- --TODO 4.在这里添加读取发射车数量定义并自动生成相应数量发射车的判断逻辑
                if _name == "S-200_Launcher" then --TODO 这是该防空系统发射车的modelname,也就是你在防空模板那里写的发射车的"name"
                    _launchers = ctld.S200Launchers --TODO 这里填你在 3. 里面定义的launcher数量变量名
                end
                ---------------------------------
                ...
                table.insert(_posArray, _crate.crateUnit:getPoint())
                table.insert(_typeArray, _name)
            end
        end
        ...
        if _activeLaunchers + 1 > _allowed then
            trigger.action.outTextForCoalition(_heli:getCoalition(), "超过防空阵地总数限制!目前防空阵地不能超过".._allowed.."个", 10)
            return
        else
            ...
            ctld.processCallback({unit = _heli, crate = _nearestCrate , spawnedGroup = _spawnedGroup, action = "spawnAASystem"})

            trigger.action.outTextForCoalition(_heli:getCoalition(), ctld.getPlayerNameOrType(_heli) .. "成功部署了一个".._aaSystem.name.."阵地. \n\n防空阵地总数的限制为".._allowed.."个\n目前已有"..(_activeLaunchers+1).."个", 10)
        ...
        local _txt = string.format("无法部署 %s!\n\n它需要 %d 箱而目前仅有 %d 箱,或者箱子之间的距离超过300m", _nearestCrate.details.desc, _nearestCrate.details.cratesRequired, #_nearbyMultiCrates)
        ctld.displayMessageToGroup(_heli, _txt, 20)
    end
end
            ...
            if _unit ~= nil then
                ...
                if _groupId then
                    ...
                    if ctld.addedTo[tostring(_groupId)] == nil then
                        ...
                        local _rootPath = missionCommands.addSubMenuForGroup(_groupId, "吊运菜单")

                        local _unitActions = ctld.getUnitActions(_unit:getTypeName())
                        ctld.logTrace(string.format("_unitActions=%s", ctld.p(_unitActions)))

                        missionCommands.addCommandForGroup(_groupId, "检查货物", _rootPath, ctld.checkTroopStatus, { _unitName })

                        if _unitActions.troops then

                            local _troopCommandsPath = missionCommands.addSubMenuForGroup(_groupId, "步兵运送", _rootPath)

                            missionCommands.addCommandForGroup(_groupId, "卸载货箱/放下步兵", _troopCommandsPath, ctld.unloadExtractTroops, { _unitName })
                            ...
                            for _,_loadGroup in pairs(ctld.loadableGroups) do
                                ...
                                if not _loadGroup.side or _loadGroup.side == _unit:getCoalition() then
                                    ...
                                    if _transportLimit >= _loadGroup.total then
                                        missionCommands.addCommandForGroup(_groupId, "装载".._loadGroup.name, _troopCommandsPath, ctld.loadTroopsFromZone, { _unitName, true,_loadGroup,false })
                                    end
                                end
                            end
                            ...
                            if ctld.unitCanCarryVehicles(_unit) then
                                ...
                                local _vehicleCommandsPath = missionCommands.addSubMenuForGroup(_groupId, "载具/FOB 运输", _rootPath)

                                missionCommands.addCommandForGroup(_groupId, "卸载载具", _vehicleCommandsPath, ctld.unloadTroops, { _unitName, false })
                                missionCommands.addCommandForGroup(_groupId, "装载/部署载具", _vehicleCommandsPath, ctld.loadTroopsFromZone, { _unitName, false,"",true })

                                if ctld.enabledFOBBuilding and ctld.staticBugWorkaround == false then

                                    missionCommands.addCommandForGroup(_groupId, "装载/部署FOB货箱", _vehicleCommandsPath, ctld.loadUnloadFOBCrate, { _unitName, false })
                                end
                                missionCommands.addCommandForGroup(_groupId, "检查货物", _vehicleCommandsPath, ctld.checkTroopStatus, { _unitName })
                            end
                        ...
                        if (ctld.enabledFOBBuilding or ctld.enableCrates) and _unitActions.crates then
                            ...
                            local _crateCommands = missionCommands.addSubMenuForGroup(_groupId, "装卸指令", _rootPath)
                            if ctld.hoverPickup == false then
                                if  ctld.slingLoad == false then
                                    missionCommands.addCommandForGroup(_groupId, "装载附近货箱", _crateCommands, ctld.loadNearbyCrate,  _unitName )
                                end
                            end

                            if ctld.slingLoad == false then
                                missionCommands.addCommandForGroup(_groupId, "卸载附近货箱", _crateCommands, ctld.dropSlingCrate, { _unitName })
                            end

                            missionCommands.addCommandForGroup(_groupId, "部署货箱", _crateCommands, ctld.unpackCrates, { _unitName })

                            missionCommands.addCommandForGroup(_groupId, "列出附近货箱", _crateCommands, ctld.listNearbyCrates, { _unitName })

                            if ctld.enabledFOBBuilding then
                                missionCommands.addCommandForGroup(_groupId, "列出FOBs", _crateCommands, ctld.listFOBS, { _unitName })
                            end
                        end


                        if ctld.enableSmokeDrop then
                            local _smokeMenu = missionCommands.addSubMenuForGroup(_groupId, "烟雾标记", _rootPath)
                            missionCommands.addCommandForGroup(_groupId, "释放红色烟雾", _smokeMenu, ctld.dropSmoke, { _unitName, trigger.smokeColor.Red })
                            missionCommands.addCommandForGroup(_groupId, "释放蓝色烟雾", _smokeMenu, ctld.dropSmoke, { _unitName, trigger.smokeColor.Blue })
                            missionCommands.addCommandForGroup(_groupId, "释放橙色烟雾", _smokeMenu, ctld.dropSmoke, { _unitName, trigger.smokeColor.Orange })
                            missionCommands.addCommandForGroup(_groupId, "释放绿色烟雾", _smokeMenu, ctld.dropSmoke, { _unitName, trigger.smokeColor.Green })
                        end
